...
export default function PagosPage() {
  const [payments,setPayments]=useState([]); const [patients,setPatients]=useState([]); const [treatments,setTreatments]=useState([]); const [error,setError]=useState('')
  const [form,setForm]=useState({patient_id:'',treatment_id:'',type:'income',concept:'',amount:'',method:'cash'})
  const load=useCallback(async()=>{const [a,b]=await Promise.all([api.get('/payments/'),api.get('/patients/')]);setPayments(a.data);setPatients(b.data);const treatmentLists=await Promise.all(b.data.map(patient=>api.get(`/patients/${patient.id}/treatments`)));setTreatments(treatmentLists.flatMap(response=>response.data))},[])
  useEffect(()=>{
  ...
  const create=async(e)=>{e.preventDefault();setError('');try{await api.post('/payments/',{...form,patient_id:form.patient_id?Number(form.patient_id):null,treatment_id:form.treatment_id?Number(form.treatment_id):null,amount:Number(form.amount)});setForm({patient_id:'',treatment_id:'',type:'income',concept:'',amount:'',method:'cash'});load()}catch(requestError){setError(requestError.response?.data?.detail||'No fue posible registrar el movimiento.')}}
  return <div className="space-y-6"><header><h1 className="text-3xl font-bold text-white">Pagos y Caja</h1><p className="text-zinc-400">Ingresos, egresos y movimientos por paciente.</p></header>
    ...
    <div className="grid lg:grid-cols-[360px_1fr] gap-6"><Card className="glass border-white/10"><CardHeader><CardTitle className="text-white">Nuevo movimiento</CardTitle></CardHeader><CardContent><form onSubmit={create} className="space-y-3">
      ...
      <div><Label>Paciente (opcional)</Label><select value={form.patient_id} onChange={e=>setForm({...form,patient_id:e.target.value,treatment_id:''})} className="mt-1 w-full bg-zinc-900 border border-white/10 rounded-md p-2"><option value="">General</option>{patients.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
      {form.patient_id&&form.type==='income'&&<div><Label>Tratamiento al que se aplica</Label><select value={form.treatment_id} onChange={e=>{const item=treatments.find(treatment=>treatment.id===Number(e.target.value));setForm({...form,treatment_id:e.target.value,concept:item?`Abono · ${item.name}`:form.concept})}} className="mt-1 w-full bg-zinc-900 border border-white/10 rounded-md p-2"><option value="">Pago general del paciente</option>{patientTreatments.map(item=><option key={item.id} value={item.id}>{item.name} · saldo {money(item.balance_amount)}</option>)}</select></div>}
      ...
      <div><Label>Método</Label><select value={form.method} onChange={e=>setForm({...form,method:e.target.value})} className="mt-1 w-full bg-zinc-900 border border-white/10 rounded-md p-2"><option value="cash">Efectivo</option><option value="card">Tarjeta</option><option value="transfer">Transferencia</option></select></div>{error&&<p className="rounded-lg border border-red-500/20 bg-red-500/10 p-3 text-sm text-red-300">{error}</p>}<Button className="w-full"><Plus className="w-4 h-4 mr-2" />Registrar</Button>
