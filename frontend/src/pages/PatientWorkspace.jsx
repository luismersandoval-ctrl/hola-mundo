...
function SignaturePad({ onChange }) {
  ...
  const move = (event) => { if(!drawing.current)return;const ctx=canvasRef.current.getContext('2d');const p=point(event);ctx.lineWidth=2.2;ctx.lineCap='round';ctx.strokeStyle='#18181b';ctx.lineTo(p.x,p.y);ctx.stroke() }
  ...
  return <div><canvas ref={canvasRef} width="700" height="180" onPointerDown={start} onPointerMove={move} onPointerUp={end} onPointerCancel={end} className="h-40 w-full touch-none rounded-xl border border-zinc-300 bg-white" aria-label="Área para firma manuscrita"/><Button type="button" variant="outline" size="sm" onClick={clear} className="mt-2 border-white/10"><Eraser className="mr-2 h-4 w-4"/>Limpiar firma</Button></div>
}
