...
export default function AgendaPage(){
  ...
  const [appointments,setAppointments]=useState([]), [patients,setPatients]=useState([]), [professionals,setProfessionals]=useState([])
  ...
  const [newPatient,setNewPatient]=useState({first_name:'',first_surname:'',phone_country_code:'+57',phone:'',email:'',gender:'unspecified'})
  const [form,setForm]=useState({patient_id:'',date:dateKey(new Date()),time:'08:00',period:'AM',reason:'',professional_user_id:'',duration_minutes:30})
  const load=useCallback(async()=>{const [a,p,d]=await Promise.all([api.get('/appointments/'),api.get('/patients/'),api.get('/professionals/')]);setAppointments(a.data);setPatients(p.data);setProfessionals(d.data)},[])
  useEffect(()=>{
  ...
  const move=(direction)=>{const next=new Date(anchor);view==='day'?next.setDate(next.getDate()+direction):view==='week'?next.setDate(next.getDate()+7*direction):next.setMonth(next.getMonth()+direction);setAnchor(next)}
  const selectCell=(day,hour,minute=0)=>{setForm({...form,date:dateKey(day),time:`${pad(hour)}:${pad(minute)}`,period:hour>=12?'PM':'AM'});setError('');setScheduleOpen(true)}
  const create=async(e)=>{e.preventDefault();if(submitting)return;if(availabilityPending){setError('Espera mientras verificamos la disponibilidad del profesional.');return}if(!selectedSlotAvailable){setError('El profesional ya tiene una cita que cruza este horario. Selecciona un bloque disponible.');return}setSubmitting(true);setError('');try{let patientId=form.patient_id;if(newMode){const r=await api.post('/patients/',newPatient);patientId=r.data.id}const professional=professionals.find(p=>p.id===Number(form.professional_user_id));if(!patientId)throw new Error('Selecciona un paciente registrado o crea un paciente nuevo.');const localDateTime=`${form.date}T${normalizedTime()}:00`;await api.post(`/patients/${patientId}/appointments`,{date:localDateTime,reason:form.reason,professional_user_id:professional?.id||null,professional:professional?.display_name||'',duration_minutes:Number(form.duration_minutes),status:'pending'});setForm({...form,patient_id:'',reason:'',duration_minutes:30});setNewPatient({first_name:'',first_surname:'',phone_country_code:'+57',phone:'',email:'',gender:'unspecified'});setNewMode(false);setPatientSearch('');setScheduleOpen(false);load()}catch(err){setError(err.response?.data?.detail||err.message||'Completa paciente, fecha y hora válidos.');load()}finally{setSubmitting(false)}}
  const changeStatus=async(item,status)=>{await api.put(`/appointments/${item.id}`,{date:item.date,reason:item.reason,professional:item.professional,professional_user_id:item.professional_user_id,duration_minutes:item.duration_minutes,status});load()}
  return <div className="min-h-screen p-3 md:p-6"><div className="mx-auto max-w-[1600px] space-y-4">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-bold text-white">Agenda</h1><p className="text-zinc-400">Disponibilidad y citas por profesional.</p></div><div className="flex flex-wrap gap-2"><div className="flex"><Button size="sm" variant={hourMode==='12'?'default':'outline'} onClick={()=>setHourMode('12')} className="rounded-r-none">12 h</Button><Button size="sm" variant={hourMode==='24'?'default':'outline'} onClick={()=>setHourMode('24')} className="rounded-l-none">24 h</Button></div>{[['day','Día'],['week','Semana'],['month','Mes']].map(([key,label])=><Button key={key} variant={view===key?'default':'outline'} onClick={()=>setView(key)} className={view===key?'':'border-white/10'}>{label}</Button>)}</div></header>
    <Card className="glass border-white/10 overflow-hidden"><CardHeader className="flex-row flex-wrap items-center justify-between gap-3"><div><CardTitle className="text-white capitalize">{view==='day'?'Agenda diaria':view==='week'?'Agenda semanal':'Calendario mensual'}</CardTitle><p className="text-sm text-zinc-500 capitalize">{anchor.toLocaleDateString(locale,{month:'long',year:'numeric'})} · Pulsa una casilla para agendar</p></div><div className="flex gap-1"><Button size="icon" variant="outline" onClick={()=>move(-1)} className="border-white/10"><ChevronLeft/></Button><Button variant="outline" onClick={()=>setAnchor(new Date())} className="border-white/10">Hoy</Button><Button size="icon" variant="outline" onClick={()=>move(1)} className="border-white/10"><ChevronRight/></Button></div></CardHeader><CardContent className="p-2 sm:p-4"><div className="mb-3 flex flex-wrap gap-x-4 gap-y-2 rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2">{Object.entries(statusStyles).map(([key,item])=><span key={key} className="flex items-center gap-1.5 text-[11px] font-medium text-zinc-300"><span className={`h-2.5 w-2.5 rounded-full ${item.dot}`} />{item.label}</span>)}</div>{view==='month'?<MonthGrid anchor={anchor} appointments={appointments} names={names} selected={form.date} onSelect={selectCell} onAppointmentClick={openAppointment}/>:<TimeGrid days={days} appointments={appointments} names={names} hourMode={hourMode} locale={locale} onSelect={selectCell} onStatus={changeStatus} onAppointmentClick={openAppointment}/>}</CardContent></Card>
    <Dialog open={scheduleOpen} onOpenChange={setScheduleOpen}><DialogContent className="glass top-3 bottom-3 max-h-none translate-y-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border-white/10 p-0 text-white sm:max-w-xl"><DialogHeader className="shrink-0 border-b border-white/10 bg-zinc-950/95 px-5 py-4 pr-12"><DialogTitle>Nueva cita · {form.date} · {form.time}</DialogTitle></DialogHeader><form onSubmit={create} className="min-h-0 space-y-3 overflow-y-auto overscroll-contain px-5 pb-6 pt-2 [scrollbar-gutter:stable]">
      ...
      {!newMode?<div className="space-y-2"><Label>Nombre del paciente</Label><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-zinc-500"/><Input autoFocus value={patientSearch} onChange={e=>setPatientSearch(e.target.value)} placeholder="Escribe nombre, teléfono o correo" className="pl-9 bg-white/5 border-white/10"/></div><div role="listbox" aria-label="Seleccionar paciente" className="max-h-32 space-y-1 overflow-y-auto rounded-md border border-white/10 bg-zinc-950 p-1">{filtered.length===0?<p className="p-3 text-center text-xs text-zinc-500">No hay resultados para la búsqueda.</p>:filtered.map(p=>{const selected=Number(form.patient_id)===p.id;return <button key={p.id} type="button" role="option" aria-selected={selected} onClick={()=>{setForm({...form,patient_id:p.id});setError('')}} className={`w-full rounded px-3 py-2 text-left text-xs transition ${selected?'bg-primary text-primary-foreground ring-1 ring-primary':'bg-white/[0.03] text-zinc-300 hover:bg-white/10'}`}><span className="block font-medium">{p.name}</span>{(p.phone||p.email)&&<span className={`block truncate text-[10px] ${selected?'opacity-80':'text-zinc-500'}`}>{p.phone?`${p.phone_country_code||''} ${p.phone}`:p.email}</span>}</button>})}</div>{form.patient_id&&<p className="text-[11px] text-emerald-400">Paciente seleccionado: {names[Number(form.patient_id)]}</p>}</div>:<div className="grid gap-2 rounded-xl border border-primary/20 bg-primary/5 p-3 sm:grid-cols-2"><div><Label>{t('firstName')}</Label><Input autoFocus required value={newPatient.first_name} onChange={e=>setNewPatient({...newPatient,first_name:e.target.value})} className="mt-1 bg-white/5 border-white/10"/></div><div><Label>{t('firstSurname')} <span className="text-zinc-500">({t('optional')})</span></Label><Input value={newPatient.first_surname} onChange={e=>setNewPatient({...newPatient,first_surname:e.target.value})} className="mt-1 bg-white/5 border-white/10"/></div><PhoneInput countryCode={newPatient.phone_country_code} phone={newPatient.phone} onCountryCodeChange={value=>setNewPatient({...newPatient,phone_country_code:value})} onPhoneChange={value=>setNewPatient({...newPatient,phone:value})} placeholder={t('phone')}/><Input type="email" placeholder={t('email')} value={newPatient.email} onChange={e=>setNewPatient({...newPatient,email:e.target.value})} className="bg-white/5 border-white/10"/></div>}
